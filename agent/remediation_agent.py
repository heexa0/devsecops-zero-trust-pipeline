...
def analyser_par_lots(vulns: list, taille_lot: int = 3) -> dict:
    ...
    return {
        ...
        "resume":           f"{len(toutes_analyses)} CVE analysées en {total_lots} lot(s) par l'IA {moteur}",
...
def afficher_resultats(analyse: dict):
    ...
    table = Table(title=f"Corrections proposées par l'IA {moteur}")
...
def generer_summary_md(analyse: dict) -> str:
    moteur = get_dernier_moteur()
    md  = f"## Rapport Agent IA — Remédiation ({moteur} — Zero Trust)\n\n"
...
if __name__ == '__main__':
    ...
    if not rapport:
        console.print("[red]trivy-report.json introuvable.[/red]")
        sys.exit(1)
