...
def build_cover(st, semgrep, trivy, zap_count):
    ...
    rows = [
        ['Étape', 'Outil', 'Résultat', 'Statut'],
        ['SAST',  'Semgrep', f'{sast_n} finding(s)', '⚠️' if sast_n else '✅'],
        ['SCA',   'Trivy',   f'{total_cve} CVE  ({critical} CRITICAL / {high} HIGH)',
         '🔴' if critical else ('⚠️' if high else '✅')],
        ['DAST',  'OWASP ZAP', f'{zap_count} alerte(s)', '⚠️' if zap_count else '✅'],
        ['IA',    'Anti-Tampering', 'Pipeline vérifié', '✅'],
        ['BUILD', 'Docker',  f'zerotrust-app:{BUILD_NUMBER}', '✅'],
        ['SIGN',  'Cosign',  'Signature keyless appliquée', '✅'],
    ]
    t = Table(rows, colWidths=[2.5*cm, 3.5*cm, 8*cm, 2.5*cm])
    t.setStyle(TableStyle([
    ...
    story.append(Paragraph('RAPPORT DE SÉCURITÉ', st['title']))
    ...
    story.append(Paragraph(
        f'Build #{BUILD_NUMBER} — {datetime.now().strftime("%d %B %Y à %H:%M")}',
        st['sub']
    ))
...
def section_sast(st, semgrep):
    ...
    if not findings:
        ...
        return story
    data = [['Fichier', 'Règle / Check ID', 'Sévérité', 'Ligne']]
...
def section_sca(st, trivy):
    story = [Paragraph('2 — Analyse des Dépendances (SCA — Trivy)', st['h1'])]
    vulns = [v for r in trivy.get('Results', []) for v in (r.get('Vulnerabilities') or [])]
    if not vulns:
        story.append(Paragraph('✅ Aucune CVE détectée.', st['ok']))
        return story
    data = [['CVE', 'Package', 'Installée', 'Corrigée', 'Sévérité']]
    for v in vulns[:30]:
...
def section_remediation(st, remediation):
    story = [Paragraph('3 — Remédiation IA des CVE (Claude Haiku)', st['h1'])]
    fixes = remediation.get('vulnerabilites', remediation.get('vulnerabilities', []))
    if not fixes:
        story.append(Paragraph('Aucune remédiation disponible.', st['warn']))
        return story
    data = [['CVE', 'Package', 'Priorité', 'Correction requirements.txt']]
    for f in fixes:
        data.append([
            f.get('cve', 'N/A'),
            f.get('package', 'N/A'),
...
def section_dast(st, zap):
    story = [Paragraph('4 — Analyse Dynamique (DAST — OWASP ZAP)', st['h1'])]
    alerts = [a for site in zap.get('site', []) for a in site.get('alerts', [])]
    if not zap:
        story.append(Paragraph('⚠️ Rapport ZAP non disponible.', st['warn']))
        return story
    if not alerts:
        story.append(Paragraph('✅ Aucune alerte détectée par OWASP ZAP.', st['ok']))
        return story
    ...
    for level, items in by_risk.items():
        if items:
            summary.append([level, str(len(items))])
...
def section_build(st):
    story = [Paragraph('5 — Build Docker & Signature Cosign', st['h1'])]
    data = [
        ['Propriété', 'Valeur'],
        ...
        ['Scan image',        'Trivy — exit-code 0 (résultats archivés)'],
    ]
...
def generate_pdf():
    ...
    story += build_cover(st, semgrep, trivy, zap_count)
    story += section_sast(st, semgrep)
    story += section_sca(st, trivy)
    story += section_remediation(st, remediation)
    ...
    story.append(Paragraph(
        f'Rapport généré automatiquement — Pipeline DevSecOps Zero Trust '
    ...
    doc.build(story)
    console.print(f'[green bold]Rapport PDF généré : {OUTPUT_PATH}[/green bold]')
